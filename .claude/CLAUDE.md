...
- 2026-06: Confirmation key handling diverges by mechanism. Reusable `ConfirmFrame` / `InputConfirm` require an explicit `y`/`Y` and ignore `Enter`. The apply confirmation uses Enter/Esc parity instead — `Enter` = yes, `Esc` = no — per owner decision. Don't "fix" apply to match the frames without checking; the divergence is intentional.
