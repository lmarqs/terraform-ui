...
- 2026-06: y/n confirmations require an explicit `y`/`Y` — NEVER accept `Enter`. The `Enter` that launches a command leaks into the alt-screen TUI and would auto-confirm a destructive op. `ConfirmFrame` and `InputConfirm` already enforce this; hand-rolled confirm handlers must match.
- 2026-06: Confirmation is a TUI-only gate (`ExecService.Apply` ignores `AutoApprove` — terraform-exec always applies non-interactively). So non-interactive apply (`-ci`/piped/no TTY) without `-auto-approve` must fail fast at the CLI boundary, not hang waiting for a prompt that can never appear.
