...
### `InvalidateState()` on the Service interface

Rejected -- exposes "there is a cache" as a concept plugins must understand. Leaks adapter internals into the core contract.

### Separate `CacheControl` interface

Rejected -- same problem with extra indirection. Plugins still learn about caching.

### Variadic functional options on existing methods (chosen)

`SkipCache()` is one option among potentially others. The method signature stays clean; the hint is optional. Internally, the adapter decides what "skip cache" means.
