...
func (m *mockService) Taint(_ context.Context, addr string) error {
...
func TestPlugin_WhenCreated_ShouldHaveCorrectMetadata(t *testing.T) {
	...
	if p.Description() != "Mark resources for recreation on next apply" {
		t.Errorf("Description() = %q, want %q", p.Description(), "Mark resources for recreation on next apply")
	}
	if p.Ready() {
...
func TestPlugin_WhenConfirmed_ShouldExecuteTaint(t *testing.T) {
	tests := []struct {
		name        string
		addresses   []string
		taintErr    error
		...
		{"ShouldTaintSingleAddress", []string{"aws_instance.web"}, nil, sdk.StatusDone, 1},
		{"ShouldTaintMultipleAddresses", []string{"aws_instance.a", "aws_instance.b"}, nil, sdk.StatusDone, 2},
		{"ShouldSetErrorOnFailure", []string{"aws_instance.web"}, errors.New("taint failed"), sdk.StatusError, 0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := &mockService{taintErr: tt.taintErr}
			p := newTestPlugin(svc)
			p.SetTargets(tt.addresses)
			...
			startMsg := startCmd()
			if _, ok := startMsg.(taintStartMsg); !ok {
				t.Fatalf("expected taintStartMsg, got %T", startMsg)
			}

			_, execCmd := p.Update(startMsg)
			if execCmd == nil {
				t.Fatal("Update(taintStartMsg) should return exec cmd")
			}
			...
			var result taintResultMsg
			found := false
			for _, subCmd := range batchMsg {
				if subCmd == nil {
					continue
				}
				if r, ok := subCmd().(taintResultMsg); ok {
					result = r
					found = true
				}
			}
			if !found {
				t.Fatal("batch did not contain taintResultMsg")
			}
			...
			if len(p.tainted) != tt.wantTainted {
				t.Errorf("tainted count = %d, want %d", len(p.tainted), tt.wantTainted)
			}
...
func TestPlugin_WhenViewRendered_ShouldShowCorrectContent(t *testing.T) {
	tests := []struct {
		...
		{"ShouldShowSuccessInDone", func(p *Plugin) {
			...
			p.tainted = []string{"aws_instance.web"}
		}, ""},
		{"ShouldShowMultiSuccessInDone", func(p *Plugin) {
			p.status = sdk.StatusDone
			p.tainted = []string{"a", "b", "c"}
		}, ""},
...
func TestPlugin_WhenTaintPartiallyFails_ShouldReportTaintedAndError(t *testing.T) {
	callCount := 0
	svc := &mockService{}
	svc.taintErr = nil
	p := newTestPlugin(svc)
	...
	// Override service with a custom one that fails on second call
	...
	if len(p.tainted) != 1 {
		t.Errorf("tainted = %d, want 1 (first succeeded before failure)", len(p.tainted))
	}
}
...
func (m *failOnNthService) Taint(_ context.Context, addr string) error {
	m.calls++
	if m.calls >= m.failOn {
		return errors.New("taint failed on " + addr)
	}
	return nil
}
