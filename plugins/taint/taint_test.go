...
func TestPlugin_Lifecycle(t *testing.T) {
	...
	if p.ID() != "taint" {
		t.Errorf("ID() = %q, want %q", p.ID(), "taint")
	}
	if p.Name() != "Taint" {
		t.Errorf("Name() = %q, want %q", p.Name(), "Taint")
	}
	if p.Description() == "" {
...
func TestPlugin_ImplementsCancellableAndBusy(t *testing.T) {
	...
	if _, ok := p.(sdk.Cancellable); !ok {
		t.Error("taint must implement sdk.Cancellable (promoted from ActionRunner)")
	}
	if _, ok := p.(sdk.Busy); !ok {
		t.Error("taint must implement sdk.Busy (promoted from ActionRunner)")
	}
}

// TestPlugin_DoesNotImplementStdoutEmitter pins the contract: taint emits no
// stdout content (success/failure flows through stderr + exit code).
func TestPlugin_DoesNotImplementStdoutEmitter(t *testing.T) {
	p := New(&sdktest.MockService{})
	if _, ok := p.(sdk.StdoutEmitter); ok {
		t.Error("taint must not implement sdk.StdoutEmitter (no stdout content)")
	}
}
...
// sdk.RequestInputMsg and feeding every other message back through Update
// until the runner reaches a terminal status.
...
func TestPlugin_Activate_AutoConfirm(t *testing.T) {
	t.Run("runs without prompting and reports success on stderr", func(t *testing.T) {
		p, _ := newTestPlugin(&sdktest.MockService{})
		cmd := p.Activate(Input{Addrs: []string{"aws_instance.web"}, AutoConfirm: true})
		if cmd == nil {
		...
		if got := string(p.Stderr()); got != "✓ Tainted aws_instance.web\n" {
			t.Errorf("Stderr() = %q, want the tainted line", got)
		}
		if p.ExitCode() != 0 {
	...
	t.Run("runs without prompting and reports failure on stderr", func(t *testing.T) {
		svc := &sdktest.MockService{TaintFn: func(context.Context, string) error {
			return errors.New("lock held")
		}}
		p, _ := newTestPlugin(svc)
		runHeadless(t, p, p.Activate(Input{Addrs: []string{"aws_instance.web"}, AutoConfirm: true}))
		if p.Status() != sdk.StatusError {
			t.Fatalf("status = %v, want Error", p.Status())
		}
		if got := string(p.Stderr()); got != "aws_instance.web: lock held\n" {
...
func TestPlugin_Spec(t *testing.T) {
	t.Run("metadata wires terraform taint semantics", func(t *testing.T) {
		p, _ := newTestPlugin(&sdktest.MockService{})
		p.addresses = []string{"a"}
		spec := p.spec()
		if spec.Name != "taint" {
			t.Errorf("Name = %q, want taint", spec.Name)
		}
		if !spec.OfferPlan {
			t.Error("taint should offer the plan shortcut on success")
		}
		if spec.ErrorLabel != "Taint failed" {
	...
	t.Run("Run taints every address in order", func(t *testing.T) {
		var calls []string
		svc := &sdktest.MockService{TaintFn: func(_ context.Context, addr string) error {
	...
	t.Run("labels reflect single vs multiple addresses", func(t *testing.T) {
		...
		if got := p.spec().Running(); got != "Tainting aws_instance.web" {
			t.Errorf("Running() = %q", got)
		}
		if got := p.spec().Done([]string{"aws_instance.web"}); got != "✓ Tainted aws_instance.web" {
			t.Errorf("Done(single) = %q", got)
		}
		p.addresses = []string{"a", "b", "c"}
		if got := p.spec().Running(); got != "Tainting 3 resources" {
			t.Errorf("Running(multi) = %q", got)
		}
		if got := p.spec().Done([]string{"a", "b", "c"}); got != "✓ Tainted 3 resources" {
...
func TestPlugin_Update(t *testing.T) {
	...
	// Async messages are consumed by the runner (timer tick while idle → nil).
	if _, cmd := p.Update(ui.TimerTickMsg{}); cmd != nil {
		t.Error("idle timer tick should be handled with no cmd")
	}

	// Keys route to the runner's standard key handling (esc in idle deactivates).
...
func TestPlugin_View_DelegatesToRunner(t *testing.T) {
	p, _ := newTestPlugin(&sdktest.MockService{})
	p.Activate(Input{Addrs: []string{"a"}}) // arms → Idle view
