...
// stdout content today (the action's success/failure flows through stderr +
// exit code). Adding StdoutEmitter would be a behavior change; this test
// guards against accidental implementation.
...
func TestPlugin_WhenConfirmed_ShouldExecuteTaint(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := &sdktest.MockService{
				TaintFn: func(_ context.Context, _ string) error {
					return tt.taintErr
				},
			}
			p, _ := newTestPlugin(svc)
